...
namespace DynamicCode.Extensions
{
    public static class StringExtension
    {
        ...
        public static ulong Crc64(this byte[] bytes, ulong crc = 0)
        {
            for (var j = 0; j < bytes.Length; j++)
            {
                crc = Table[(byte)(crc ^ bytes[j])] ^ (crc >> 8);
            }
            return crc;
        }

        public static ulong Crc64(this string value, ulong crc = 0)
        {
        ...
        public static dynamic Build(this string code)
        {
            return new DynamicAssembly(code);
        }
